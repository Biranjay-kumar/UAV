...
int Drone::time_to_reach(const Base &base)
{
	return static_cast<int>(sqrt(pow(base.x - x, 2) + pow(base.y - y, 2)));
}
...
bool Drone::make_delivery(Base &base)
{
	...
	if (total_time + travel_time > base.time_limit)
	{
		std::cout << "Failed to deliver to base " << base.id << " within time limit.\n";
		return false;
	}
	...
	std::cout << "Delivered " << deliverable_weight << " units to base " << base.id << ".\n";
	return true;
}

void Drone::make_multiple_trips(Base &base, const std::vector<std::vector<int>> &time_matrix, std::vector<Base> &bases)
{
	while (base.weight_required > 0)
	{
		bool delivered = false;
		...
		if (!delivered && all_drones.size() < max_drones)
		{
			Drone new_drone(0, 0, all_drones.front().max_capacity);
			delivered = new_drone.make_delivery(base);
		}

		if (!delivered)
		{
			std::cout << "Delivery to base " << base.id << " failed.\n";
			break;
		}
	}
}
